class AdminCategoriesManager {
    ...
    initImageUpload() {
        const uploadArea = document.getElementById('sectionImageUploadArea');
        const fileInput = document.getElementById('sectionImageFile');
        const preview = document.getElementById('sectionImagePreview');
        const removeBtn = document.getElementById('removeSectionImage');
        const hiddenUrl = document.getElementById('sectionImageUrl');
        ...
        fileInput.addEventListener('change', async (e) => {
            ...
            const result = await this.uploadSectionImage(file);
            if (result.success) {
    ...
    async uploadSectionImage(file) {
        const formData = new FormData();
        formData.append('file', file);
        try {
            const resp = await fetch(`${this.API_BASE}/api/upload/section`, {
                method: 'POST',
                body: formData,
                headers: { 'Authorization': `Bearer ${this.authToken}` }
            });
    ...
    openCategoryModal(categoryId = null) {
        ...
        document.getElementById('categoryCode').dataset.manual = '';
        // Сброс изображения
    ...
    loadCategoryData(categoryId) {
        ...
        if (category.image_url) {
            hiddenUrl.value = category.image_url;
            preview.querySelector('img').src = category.image_url;
            preview.style.display = 'block';
        } else {
            hiddenUrl.value = '';
            preview.style.display = 'none';
    ...
    async handleCategorySubmit(e) {
        ...
        const categoryData = {
            ...
            image_url: formData.get('image_url') || ''   // новое поле
        };
