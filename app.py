    ...
    if 'id' in product_data and product_data['id']:
        ...
        with db.get_db() as conn:
            conn.execute('''UPDATE products SET name=?, code=?, category=?, section=?, price=?, old_price=?, badge=?,
                recommended=?, description=?, specifications=?, status=?, stock=?, images=?, color_variants=?, updated_at=?, is_price_on_request=?
                ...
                json.dumps(product_data.get('color_variants',[])),now,1 if product_data.get('is_price_on_request') else 0,product_id))
        return product_id
    else:
        with db.get_db() as conn:
            ...
            conn.execute('''INSERT INTO products (id,name,code,category,section,price,old_price,badge,recommended,description,
                specifications,status,stock,images,color_variants,created_at,updated_at,is_price_on_request)
