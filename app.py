...
def set_product_flag(product_id, flag_name, value):
    ...
    if pid not in flags:
        flags[pid] = {}
    flags[pid][flag_name] = value
    save_product_flags(flags)

# ========== МИГРАЦИЯ СТАРЫХ ДАННЫХ ИЗ ./data В /data ==========
OLD_DATA_DIR = os.path.join(BASE_DIR, 'data')
if os.path.exists(OLD_DATA_DIR) and not os.path.exists(DB_PATH):
    print("Обнаружены старые данные в ./data, копирую в /data...")
    try:
        # Копируем БД
        old_db = os.path.join(OLD_DATA_DIR, 'ma_furniture.db')
        if os.path.exists(old_db):
            shutil.copy2(old_db, DB_PATH)
            print("БД скопирована")
        # Копируем JSON-файлы
        for item in ['products', 'orders', 'sections.json', 'background.json']:
        ...
        # Копируем загруженные изображения, если они были в static/uploads
        old_uploads = os.path.join(STATIC_DIR, 'uploads', 'products')
        if os.path.exists(old_uploads) and os.listdir(old_uploads):
            for f in os.listdir(old_uploads):
                src_f = os.path.join(old_uploads, f)
                dst_f = os.path.join(UPLOAD_FOLDER, f)
                if os.path.isfile(src_f) and not os.path.exists(dst_f):
                    shutil.copy2(src_f, dst_f)
            print("Изображения скопированы")
        print("Миграция старых данных завершена")
    except Exception as e:
...
# Миграция из JSON (если есть файлы, а БД пуста)
